...
def MLS(image, input_points, output_points):
    ...
    start = time.time()
    #仿射变形,速度最快
    affine = mls_affine_deformation(vy, vx, input_points, output_points, alpha=1)
    UX =  affine[0,:,:].astype(np.float32)
    UY =  affine[1,:,:].astype(np.float32)
    aug1 = cv2.remap(image, UX, UY, interpolation=cv2.INTER_LINEAR)
    
    end1 = time.time()
    print("MLS time cost:{} s".format(end1 - start))
    
    
    #相似变形,速度最慢
    similar = mls_similarity_deformation(vy, vx, input_points, output_points, alpha=1)
    UX =  affine[0,:,:].astype(np.float32)
    UY =  affine[1,:,:].astype(np.float32)
    aug2 = cv2.remap(image, UX, UY, interpolation=cv2.INTER_LINEAR)
    
    end2 = time.time()
    print("MLS time cost:{} s".format(end2 - end1))


    #网格变形,速度中间和最慢差不多
    rigid = mls_rigid_deformation(vy, vx, input_points, output_points, alpha=1)
    UX =  affine[0,:,:].astype(np.float32)
    UY =  affine[1,:,:].astype(np.float32)
    aug3 = cv2.remap(image, UX, UY, interpolation=cv2.INTER_LINEAR)

    end3 = time.time()
    print("MLS time cost:{} s".format(end3 - end2))

   
    
    end = time.time()
    print("MLS time cost:{} s".format(end - start))
    return aug1
...
def MLS_TORCH(image, input_points, output_points):

    device = torch.device("cuda" if torch.cuda.is_available() else 'cpu')
    input_points_pytorch = torch.from_numpy(input_points).to(device)
    output_points_pytorch = torch.from_numpy(output_points).to(device)
    image_pytorch = torch.from_numpy(image).to(device)


    height, width, _ = image_pytorch.shape
    gridX = torch.arange(width, dtype=torch.int16).to(device)
    gridY = torch.arange(height, dtype=torch.int16).to(device)
    vx, vy = torch.meshgrid(gridX, gridY)
    # !!! Pay attention !!!: the shape of returned tensors are different between numpy.meshgrid and torch.meshgrid
    vx, vy = vx.transpose(0, 1), vy.transpose(0, 1)


    
    start = time.time()
    #仿射变形
    affine = mls_affine_deformation_pt(vy, vx, input_points_pytorch, output_points_pytorch, alpha=1)
    ...
    end1 = time.time()
    print("MLS TORCH time cost:{} s".format(end1 - start))
 

    
    #相似变形
    similar = mls_similarity_deformation_pt(vy, vx, input_points_pytorch, output_points_pytorch, alpha=1)
    similar = similar.cpu().numpy()
    UX =  similar[0,:,:].astype(np.float32)
    UY =  similar[1,:,:].astype(np.float32)
    aug2 = cv2.remap(image, UX, UY, interpolation=cv2.INTER_LINEAR)
    
    end2 = time.time()
    print("MLS TORCH time cost:{} s".format(end2 - end1))
 



    #网格变形
    rigid = mls_rigid_deformation_pt(vy, vx, input_points_pytorch, output_points_pytorch, alpha=1)
    rigid = rigid.cpu().numpy()
    UX =  rigid[0,:,:].astype(np.float32)
    UY =  rigid[1,:,:].astype(np.float32)
    aug3 = cv2.remap(image, UX, UY, interpolation=cv2.INTER_LINEAR)
    
    end3 = time.time()
    print("MLS TORCH time cost:{} s".format(end3 - end2))
 

    end = time.time()
    print("MLS TORCH time cost:{} s".format(end - start))
    return aug1
...
def demo():
    ...
    UX =  affine[0,:,:].astype(np.float32)
    UY =  affine[1,:,:].astype(np.float32)
    aug1 = cv2.remap(image, UX, UY, interpolation=cv2.INTER_LINEAR).astype(np.uint8)
 

    similar = mls_similarity_deformation(vy, vx, p, q, alpha=1)
    UX =  similar[0,:,:].astype(np.float32)
    UY =  similar[1,:,:].astype(np.float32)
    aug2 = cv2.remap(image, UX, UY, interpolation=cv2.INTER_LINEAR).astype(np.uint8)
 

    rigid = mls_rigid_deformation(vy, vx, p, q, alpha=1)
    UX =  rigid[0,:,:].astype(np.float32)
    UY =  rigid[1,:,:].astype(np.float32)
    aug3 = cv2.remap(image, UX, UY, interpolation=cv2.INTER_LINEAR).astype(np.uint8)
...
def demo_torch():
    ...
    p = torch.from_numpy(input_points).to(device)
    q = torch.from_numpy(output_points).to(device)
    """
    ...
    #image = cv2.imread("images/toy.jpg")
...
def demo2():
    ...
    """
    p1 = np.load("input_points_m.npy")
    q1 = np.load("output_points_m.npy")

    rigid1 = mls_rigid_deformation(vy, vx, p1, q1, alpha=1)
    UX =  rigid1[0,:,:].astype(np.float32)
    UY =  rigid1[1,:,:].astype(np.float32)
    aug1 = cv2.remap(image, UX, UY, interpolation=cv2.INTER_LINEAR)
    ...
    UX =  rigid2[0,:,:].astype(np.float32)
    UY =  rigid2[1,:,:].astype(np.float32)
    aug2 = cv2.remap(image, UX, UY, interpolation=cv2.INTER_LINEAR)
    """
...
def test():
    srcImg = cv2.imread("all.png")
    #srcImg = srcImg[:720,:720,:]
    input_points = np.load("input_points_271.npy")
    output_points = np.load("output_points_271.npy")
    
    src = srcImg.copy()
    for i in range(0,input_points.shape[0],1):
        cv2.circle(src,(int(input_points[i,0]), int(input_points[i,1])), 1, (0,0,255), cv2.MARKER_SQUARE)
        cv2.circle(src,(int(output_points[i,0]), int(output_points[i,1])), 1, (0,255,255), cv2.MARKER_CROSS)
    cv2.imwrite("src.jpg", src)
    
    aug = MLS(srcImg, input_points, output_points)
    #aug = MLS_TORCH(srcImg, input_points, output_points)

    
    for i in range(0,input_points.shape[0],1):
        cv2.circle(aug,(int(input_points[i,0]), int(input_points[i,1])), 1, (0,0,255), cv2.MARKER_SQUARE)
        cv2.circle(aug,(int(output_points[i,0]), int(output_points[i,1])), 1, (0,255,255), cv2.MARKER_CROSS)
    cv2.imwrite("aug.jpg", aug)
